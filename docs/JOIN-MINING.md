...
Save the mnemonic somewhere safe — it is the only way to recover the wallet. Your address starts
with `P` or `Q`.
...
Leave it running. It serves RPC on `http://127.0.0.1:38300`. Wait until its reported height stops
climbing and matches a public explorer before mining — a miner cannot build on a chain it has not
caught up to.
...
If your node requires an RPC token, also set `IRIUM_RPC_TOKEN`.
