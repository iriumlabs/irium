...
// -- Stage 3a: pool-side candidate-admission generation (gated, accumulate-only) --
// The pool does the role-lane VRF work under its OWN secret and tags a connected
// miner's pkh as the role solver, so real per-miner admissions flow into the node's
// candidate-admission cache. This is submission-only: it NEVER affects the daemon
// bridge or block production. Reversible: unset the env flag to stop instantly.

/// Master gate. OFF by default so deploying the binary changes nothing until the
/// operator sets IRIUM_POAWX_POOL_SUBMIT_ADMISSIONS=1; unset it to stop.
pub fn pool_admission_submit_enabled() -> bool {
    std::env::var("IRIUM_POAWX_POOL_SUBMIT_ADMISSIONS")
        .map(|v| v.trim() == "1")
        .unwrap_or(false)
}

/// One-admission-per-(height, miner, role) dedup so we submit at most one admission
/// per accepted miner per height (not per share). Pruned to recent heights.
fn pool_admission_dedup(
) -> &'static std::sync::Mutex<std::collections::BTreeSet<(u64, [u8; 20], u8)>> {
    static S: std::sync::OnceLock<
        std::sync::Mutex<std::collections::BTreeSet<(u64, [u8; 20], u8)>>,
    > = std::sync::OnceLock::new();
    S.get_or_init(|| std::sync::Mutex::new(std::collections::BTreeSet::new()))
}

/// Share-weighted role rotation: a deterministic per-(miner, height) role so distinct
/// miners land distinct roles across heights (mirrors the stage-1 soak rotation).
...
/// Build canonical admission wire bytes attributing `role` to `miner_pkh`. The pool
/// signs the VRF work with its own secret; `solver_pkh` is the attribution tag.
pub fn build_pool_candidate_admission_bytes(
    network_id: u8,
    pool_secret: &[u8; 32],
    height: u64,
    seed: &[u8; 32],
    ...
    let adm = irium_node_rs::poawx_admission::CandidateAdmissionV1::new_with_v2(
        network_id, height, *seed, cand, Some(proof),
    );
    Some(adm.serialize())
}

/// POST an admission to the node's loopback ingest; returns the node status string.
pub async fn submit_candidate_admission(rpc_base: &str, bytes: Vec<u8>) -> Option<String> {
    let client = reqwest::Client::builder().build().ok()?;
    let url = format!("{}/poawx/candidate-admission", rpc_base.trim_end_matches('/'));
    let resp = client.post(&url).body(bytes).send().await.ok()?;
    let v = resp.json::<serde_json::Value>().await.ok()?;
    v.get("status").and_then(|s| s.as_str()).map(|s| s.to_string())
}

/// Orchestrator, called on an active mining session at `height`. Submits at most one
/// admission per (height, miner, role); non-blocking (spawns the POST). No effect
/// unless enabled. NEVER touches production/attribution.
pub fn maybe_submit_pool_admission(
    network_id: u8,
    pool_secret: [u8; 32],
    rpc_base: String,
    height: u64,
    ...
    if !pool_admission_submit_enabled() {
        return;
    }
    let role = pool_role_for_miner(&miner_pkh, height);
    {
        let mut dd = pool_admission_dedup()
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        dd.retain(|(h, _, _)| *h + 64 >= height);
        if !dd.insert((height, miner_pkh, role)) {
            return; // already submitted for this miner+role at this height
        }
    }
    let bytes = match build_pool_candidate_admission_bytes(
        network_id, &pool_secret, height, &seed, miner_pkh, role,
    ) {
        Some(b) => b,
        None => return,
    };
    if let Ok(handle) = tokio::runtime::Handle::try_current() {
        handle.spawn(async move {
            match submit_candidate_admission(&rpc_base, bytes).await {
                Some(st) => eprintln!(
                    "[poawx-admission] submitted height={} miner={} role={} -> {}",
                    height,
                    hex::encode(&miner_pkh[..4]),
                    role,
                    st
                ),
                None => eprintln!(
                    "[poawx-admission] POST failed height={} miner={} role={}",
                    height,
                    hex::encode(&miner_pkh[..4]),
                    role
                ),
...
mod tests {
    ...
    // Stage 3a: the pool-generated admission (pool signs the VRF work under its own
    // secret, tags the miner's pkh) decodes to a valid admission the pool's own
    // admitted-set path accepts, attributing the role to the miner. Plus role
    // rotation is deterministic and rotates across heights.
    #[test]
    fn stage3a_pool_admission_bytes_decode_and_role_rotation() {
        let net = 3u8;
        let secret = [0x9Au8; 32];
        let seed = [0x44u8; 32];
        let height = 50_360u64;
        let miner = [0xB1u8; 20];
        let role = pool_role_for_miner(&miner, height);
        assert!(
            [ROLE_COMPUTE_CONTRIBUTOR, ROLE_VERIFY_CONTRIBUTOR, ROLE_SUPPORT_CONTRIBUTOR]
                .contains(&role)
        );
        let bytes = build_pool_candidate_admission_bytes(net, &secret, height, &seed, miner, role)
            .expect("admission bytes");
        let hx = hex::encode(&bytes);
        let (dh, ds, dc) = decode_admission_candidate(&hx).expect("admission decodes");
        assert_eq!(dh, height);
        assert_eq!(ds, seed);
        assert_eq!(dc.solver_pkh, miner, "admission attributes the role to the miner pkh");
        assert_eq!(dc.role_id, role);
        // the pool's admitted-set builder accepts it and selects the miner for the role.
        let cs = build_admitted_candidate_set(&[hx.clone()], net, height, &seed).expect("set");
        assert_eq!(cs.best_for_role(role).unwrap().solver_pkh, miner);
        // the VRF proof (pool-signed) is extractable for the (role, miner) pair.
        let proofs = build_admitted_v2_proofs(&[hx], height, &seed);
        assert!(proofs.contains_key(&(role, miner)), "pool VRF proof present");
        // rotation: deterministic, and a miner rotates across heights.
        assert_eq!(pool_role_for_miner(&miner, height), pool_role_for_miner(&miner, height));
        let across: std::collections::BTreeSet<u8> =
            (0..9).map(|d| pool_role_for_miner(&miner, height + d)).collect();
        assert!(across.len() >= 2, "a miner's role rotates across heights");
        // dedup: the same (height, miner, role) inserts once.
        let dd = pool_admission_dedup();
        {
            let mut g = dd.lock().unwrap_or_else(|e| e.into_inner());
            g.clear();
            assert!(g.insert((height, miner, role)));
            assert!(!g.insert((height, miner, role)), "dedup: one per (height,miner,role)");
            g.clear();
        }
        // gate off => orchestrator is a no-op (no panic without a tokio runtime).
        std::env::remove_var("IRIUM_POAWX_POOL_SUBMIT_ADMISSIONS");
        maybe_submit_pool_admission(net, secret, "http://127.0.0.1:1".into(), height, seed, miner);
    }
    ...
    // Phase 21C: prove the mirror's ticket-proof sybil digest is bound to prev_hash
    // EXACTLY like the node's -- byte-identical for the same prev, PROVABLY different
    // for a different prev (prev_hash genuinely participates), and the node validator
    // accepts under the matching prev and rejects under a wrong prev. Direct proof of
    // the mirror<->node prev_hash sync fix.
    #[test]
    fn phase21c_ticket_mirror_prev_hash_parity() {
        let net = 1u8;
        ...
        let mk_pm = |prev: [u8; 32]| {
            TicketProofMirror::new(
                net, 7, prev, ROLE_COMPUTE_CONTRIBUTOR, solver, 3, 300, apk, nonce, 0,
            )
        };
        let mk_nb = |prev: [u8; 32]| {
            irium_node_rs::poawx_ticket::TicketProof::new(
                net, 7, prev, irium_node_rs::poawx::ROLE_COMPUTE_CONTRIBUTOR, solver, 3,
                300, apk, nonce, 0,
            )
        };
        // Positive parity: same prev -> mirror bytes == node bytes (incl. prev-bound digest).
        assert_eq!(
            mk_pm(prev_a).serialize(),
            mk_nb(prev_a).serialize(),
            "mirror ticket-proof byte-identical to node for prev_a"
        );
        assert_eq!(
            mk_pm(prev_b).serialize(),
            mk_nb(prev_b).serialize(),
            "mirror ticket-proof byte-identical to node for prev_b"
        );
        // prev_hash genuinely participates: different prev -> different bytes on BOTH sides.
        assert_ne!(
            mk_pm(prev_a).serialize(),
            mk_pm(prev_b).serialize(),
            "mirror digest must change with prev_hash (not ignored)"
        );
        assert_ne!(
            mk_nb(prev_a).serialize(),
            mk_nb(prev_b).serialize(),
            "node digest must change with prev_hash"
        );
        // sybil_work_digest specifically matches the node's compute_sybil_digest.
        let d = mirror_compute_sybil_digest(net, &prev_a, &solver, 3, &apk, &nonce);
        let dn =
            irium_node_rs::poawx_ticket::compute_sybil_digest(net, &prev_a, &solver, 3, &apk, &nonce);
        assert_eq!(d, dn, "mirror_compute_sybil_digest == node compute_sybil_digest");
        // Node validator: accepts mirror-built proof under matching prev, rejects wrong prev.
        let parsed =
            irium_node_rs::poawx_ticket::TicketProof::deserialize(&mk_pm(prev_a).serialize()).unwrap();
        assert!(
            parsed
                .validate(
                    net,
                ...
                .is_ok(),
            "node validates mirror proof under matching prev"
        );
        assert!(
            parsed
                .validate(
                ...
                .is_err(),
            "node rejects mirror proof under wrong prev (prev_hash binding enforced)"
        );
    }
